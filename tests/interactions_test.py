...
class TestInteractions:
    ...
    class TestDraggablePage:
        ...
        def test_simple_draggable(self, driver):
            ...
            draggable_page = DraggablePage(driver, url)
            draggable_page.open()
            before, after = draggable_page.simple_drag_box()
            assert before != after, 'position of the box has not been changed'
        ...
        def test_axis_draggable(self,driver):
            ...
            assert top_x[0][0] == top_x[1][0] and int(
                top_x[1][0]) == 0, 'box position has not been changed or there has been a shift in the y-axis'
            assert left_x[0][0] != left_x[1][0] and int(
                left_x[1][0]) != 0, 'box position has not been changed or there has been a shift in the y-axis'
            assert top_y[0][0] != top_y[1][0] and int(
                top_y[1][0]) != 0, 'box position has not been changed or there has been a shift in the x-axis'
            assert left_y[0][0] == left_y[1][0] and int(
                left_y[1][0]) == 0, 'box position has not been changed or there has been a shift in the x-axis'
